<?php
/**
 * Session add more security to the basic PHP sessions.
 * Sessions are validated before beiing used to prevent session theft.
...
class Session
{
    ...
    public static function decrypt($string, $silent = false, $key = false)
    {
        ...
        if (strlen($raw) < self::$ivLength)
        {
            ...
            throw new \Exception('Session::decrypt() - '.'data length ' . strlen($raw) . " is less than iv length {".self::$ivLength."}");
        }
